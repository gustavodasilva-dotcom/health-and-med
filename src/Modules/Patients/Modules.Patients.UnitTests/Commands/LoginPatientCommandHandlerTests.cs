...
namespace Modules.Patients.UnitTests.Commands
{
    public class LoginPatientCommandHandlerTests
    {
        ...
        public async Task Handle_ShouldReturnError_WhenPatientNotFound()
        {
            ...
            _mockPatientRepository.Setup(repo => repo.GetWithEmail(command.Email)).Returns((Patient)null);
        ...
        public async Task Handle_ShouldReturnError_WhenPasswordIsIncorrect()
        {
            ...
            _mockPatientRepository.Setup(repo => repo.GetWithEmail(command.Email)).Returns(patient);
            _mockPasswordHasher.Setup(ph => ph.Verify(command.Password.Trim(), patient.Password)).Returns(false);
        ...
        public async Task Handle_ShouldReturnToken_WhenLoginIsSuccessful()
        {
            ...
            _mockPatientRepository.Setup(repo => repo.GetWithEmail(command.Email)).Returns(patient);
            _mockPasswordHasher.Setup(ph => ph.Verify(command.Password.Trim(), patient.Password)).Returns(true);
            _mockTokenProvider.Setup(tp => tp.Create(patient, UserRoles.Patient)).Returns(expectedToken);
