...
namespace Modules.Doctors.UnitTests.Commands
{
    public class RegisterDoctorCommandHandlerTests
    {
        ...
        public async Task Handle_ShouldReturnError_WhenEmailAlreadyInUse()
        {
            ...
            _mockDoctorRepository.Setup(repo => repo.ExistsWithEmail(command.Email)).Returns(true);
        ...
        public async Task Handle_ShouldReturnError_WhenSsnAlreadyInUse()
        {
            ...
            _mockDoctorRepository.Setup(repo => repo.ExistsWithEmail(command.Email)).Returns(false);
            _mockDoctorRepository.Setup(repo => repo.ExistsWithSsn(command.Ssn)).Returns(true);
        ...
        public async Task Handle_ShouldReturnError_WhenRegistrationNumberIsAlreadyRegistered()
        {
            ...
            _mockDoctorRepository.Setup(repo => repo.ExistsWithEmail(command.Email)).Returns(false);
            _mockDoctorRepository.Setup(repo => repo.ExistsWithSsn(command.Ssn)).Returns(false);
            _mockDoctorRepository.Setup(repo => repo.IsRegisteredInState(command.RegistrationNumber, command.RegistrationState)).Returns(true);
        ...
        public async Task Handle_ShouldReturnSuccess_WhenRegistrationIsSuccessful()
        {
            ...
            _mockDoctorRepository.Setup(repo => repo.ExistsWithEmail(command.Email)).Returns(false);
            _mockDoctorRepository.Setup(repo => repo.ExistsWithSsn(command.Ssn)).Returns(false);
            _mockDoctorRepository.Setup(repo => repo.IsRegisteredInState(command.RegistrationNumber, command.RegistrationState)).Returns(false);
            _mockPasswordHasher.Setup(ph => ph.Hash(command.Password)).Returns("hashedPassword");
